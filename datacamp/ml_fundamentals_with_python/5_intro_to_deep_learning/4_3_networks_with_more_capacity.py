...
def experimenting_with_wider_networks(predictors, target):
    model_1 = Sequential()
...
def adding_layers_to_a_network(predictors, target):
    early_stopping_monitor = EarlyStopping(patience=2)
    model_1 = Sequential()
    model_1.add(Dense(50, activation='relu', input_shape=(10,)))
    model_1.add(Dense(2, activation='softmax'))
    model_1.compile(optimizer='adam',
                    loss='categorical_crossentropy', metrics=['accuracy'])
...
#experimenting_with_wider_networks(predictors, target)
adding_layers_to_a_network(predictors, target)