...
def plot_classifier(X, y, clf, ax=None, ticks=False, proba=False, lims=None):
    ...
    cs = plot_contours(ax, clf, xx, yy, cmap=plt.cm.coolwarm,
                       alpha=0.8, proba=proba)
    if proba:
        cbar = plt.colorbar(cs)
        cbar.ax.set_ylabel('probability of red $\Delta$ class',
...
def plot_contours(ax, clf, xx, yy, proba=False, **params):
    ...
    if proba:
        ...
        out = ax.imshow(Z, extent=(np.min(xx), np.max(xx), np.min(
            yy), np.max(yy)), origin='lower', vmin=0, vmax=1, **params)
...
def show_digit(i, lr=None):
    ...
    if lr is None:
        ...
        plt.title("label=%d, prediction=%d, proba=%.2f" %
                  (y[i], pred, pred_prob))
    plt.show()
