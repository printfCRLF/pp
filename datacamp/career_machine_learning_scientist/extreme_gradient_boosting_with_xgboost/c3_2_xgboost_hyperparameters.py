...
def tuning_eta(X, y):
    ...
    for curr_val in eta_vals:
        ...
        cv_results = xgb.cv(dtrain=housing_dmatrix, params=params, nfold=3,
                            num_boost_round=10, early_stopping_rounds=5, metrics="rmse", seed=123, as_pandas=True)
...
def tuning_colsample_bytree(X, y):
    # Create your housing DMatrix
    housing_dmatrix = xgb.DMatrix(data=X, label=y)
...
y = df.iloc[:, -1]
# tuning_eta(X, y)
# tunning_max_depth(X, y)
tuning_colsample_bytree(X, y)
